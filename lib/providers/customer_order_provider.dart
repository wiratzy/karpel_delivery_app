...
Future<Order> fetchOrderById(String token, int orderId) async {
  ...
  } catch (e, stackTrace) { // Tangkap error (e) dan stackTrace-nya
    
    // 4. Cetak error asli dan di mana terjadinya (stackTrace)
    print("❌ Terjadi error saat parsing Order.fromJson untuk orderId $orderId.");
    print("Error Asli: $e");
...
Future<void> updateOrderStatus(String token, int orderId, String status) async {
  await _apiService.updateCustomerOrderStatus(token, orderId, status);
}
}
